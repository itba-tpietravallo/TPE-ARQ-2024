...
static void renderFromBitmap(char * bitmap) {
    for (int x = 0; x < 8; x++) {
        for (int y = 0; y < 8; y++) {
            putPixel(bitmap[y] & (1 << x) ? 0x00FFFFFF : 0x0, x, y);
        }
    }
}

void render(char ascii) {
    if (ascii < 128) // otherwise it is ignored
        renderFromBitmap(font8x8_basic[(int)ascii]);
}